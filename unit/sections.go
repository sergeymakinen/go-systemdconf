...
type UnitSection struct {
	...
	// A short human readable title of the unit. This may be used by systemd (and other UIs) as a user-visible label for the unit,
	// so this string should identify the unit rather than describe it, despite the name. This string also should not just repeat
	// the unit name. "Apache2 Web Server" is a good example. Bad examples are "high-performance lightweight HTTP server" (too
	// generic) or "Apache2" (meaningless for people who do not know Apache, duplicates the unit name). systemd may use this string
	// as a noun in status messages ("Starting description...", "Started description.", "Reached target description.", "Failed
	// to start description."), so it should be capitalized, and should not be a full sentence, or a phrase with a continuous verb.
	// Bad examples include "exiting the container" or "updating the database once per day.".
	//
	...
	// After=, this unit will be stopped (or restarted) if one of the other units is explicitly stopped (or restarted).
	//
	...
	// that suddenly enters inactive state, but also one that is bound to another unit that gets skipped due to an unmet condition
	...
	// Configures dependencies similar to Wants=, but as long as this unit is up, all units listed in Upholds= are started whenever
	// found to be inactive or failed, and no job is queued for them. While a Wants= dependency on another unit has a one-time effect
	// when this units started, a Upholds= dependency on it has a continuous effect, constantly restarting the unit if necessary.
	// This is an alternative to the Restart= setting of service units, to ensure they are kept running whatever happens. The restart
	// happens without delay, and usual per-unit rate-limit applies.
	//
	// When Upholds=b.service is used on a.service, this dependency will show as UpheldBy=a.service in the property listing
	// of b.service.
	//
	// Added in version 249.
	Upholds systemdconf.Value
	...
	// dependency must be declared. It does not matter which of the two ordering dependencies is used, because stop jobs are always
	...
	// before the start-up. It does not matter if the ordering dependency is After= or Before=, in this case. It also does not matter
	...
	// commands have been invoked and they either failed or reported start-up success. Note that this includes ExecStartPost=
	...
	//
	// Added in version 249.
	OnSuccess systemdconf.Value

	// A space-separated list of one or more units to which reload requests from this unit shall be propagated to, or units from
	// which reload requests shall be propagated to this unit, respectively. Issuing a reload request on a unit will automatically
	// also enqueue reload requests on all units that are linked to it using these two settings.
	//
	// Added in version 201.
	...
	// A space-separated list of one or more units to which stop requests from this unit shall be propagated to, or units from which
	// stop requests shall be propagated to this unit, respectively. Issuing a stop request on a unit will automatically also
	// enqueue stop requests on all units that are linked to it using these two settings.
	//
	// Added in version 249.
	PropagatesStopTo systemdconf.Value
	...
	//
	// Added in version 249.
	StopPropagatedFrom systemdconf.Value

	// For units that start processes (such as service units), lists one or more other units whose network and/or temporary file
	// namespace to join. If this is specified on a unit (say, a.service has JoinsNamespaceOf=b.service), then the inverse dependency
	// (JoinsNamespaceOf=a.service for b.service) is implied. This only applies to unit types which support the PrivateNetwork=,
	// NetworkNamespacePath=, PrivateIPC=, IPCNamespacePath=, and PrivateTmp= directives (see systemd.exec for details).
	// If a unit that has this setting set is started, its processes will see the same /tmp/, /var/tmp/, IPC namespace and network
	// namespace as one listed unit that is started. If multiple listed units are already started and these do not share their namespace,
	// then it is not defined which namespace is joined. Note that this setting only has an effect if PrivateNetwork=/NetworkNamespacePath=,
	// PrivateIPC=/IPCNamespacePath= and/or PrivateTmp= is enabled for both the unit that joins the namespace and the unit
	...
	WantsMountsFor systemdconf.Value

	// Takes a value of "fail", "replace", "replace-irreversibly", "isolate", "flush", "ignore-dependencies" or "ignore-requirements".
	// Defaults to "replace". Specifies how the units listed in OnSuccess=/OnFailure= will be enqueued. See systemctl's --job-mode=
	// option for details on the possible values. If this is set to "isolate", only a single unit may be listed in OnSuccess=/OnFailure=.
	//
	// Added in version 209.
	OnSuccessJobMode systemdconf.Value
	...
	// Takes a boolean argument. Defaults to no. If yes, processes belonging to this unit will not be sent the final "SIGTERM" and
	// "SIGKILL" signals during the final phase of the system shutdown process. This functionality replaces the older mechanism
	// that allowed a program to set "argv[0][0] = '@'" as described at systemd and Storage Daemons for the Root File System, which
	// however continues to be supported.
	//
	// Added in version 255.
	SurviveFinalKillSignal systemdconf.Value
	...
	// reboot-immediate, poweroff, poweroff-force, poweroff-immediate, exit, exit-force, soft-reboot, soft-reboot-force,
	// kexec, kexec-force, halt, halt-force and halt-immediate. In system mode, all options are allowed. In user mode, only
	...
	// poweroff-force, poweroff-immediate, kexec, kexec-force, halt, halt-force and halt-immediate have the effect of powering
	// down the system, executing kexec, and halting the system respectively with similar semantics. exit causes the manager
	// to exit following the normal shutdown procedure, and exit-force causes it terminate without shutting down services.
	// When exit or exit-force is used by default the exit status of the main process of the unit (if this applies) is returned from
	// the service manager. However, this may be overridden with FailureActionExitStatus=/SuccessActionExitStatus=, see
	// below. soft-reboot will trigger a userspace reboot operation. soft-reboot-force does that too, but does not go through
	// the shutdown transaction beforehand.
	//
	// Added in version 236.
	...
	// JobTimeoutSec= specifies a timeout for the whole job that starts running when the job is queued. JobRunningTimeoutSec=
	// specifies a timeout that starts running when the queued job is actually started. If either limit is reached, the job will
	// be cancelled, the unit however will not change state or even enter the "failed" mode.
	//
	// Both settings take a time span with the default unit of seconds, but other units may be specified, see systemd.time. The
	// default is "infinity" (job timeouts disabled), except for device units where JobRunningTimeoutSec= defaults to DefaultDeviceTimeoutSec=.
	//
	// Note: these timeouts are independent from any unit-specific timeouts (for example, the timeout set with TimeoutStartSec=
	// in service units). The job timeout has no effect on the unit itself. Or in other words: unit-specific timeouts are useful
	...
	// and JobRunningTimeoutSec= above. It takes the same values as FailureAction=/SuccessAction=. Defaults to none.
	//
	// JobTimeoutRebootArgument= configures an optional reboot string to pass to the reboot system call.
	//
	// Added in version 240.
	...
	// Configure unit start rate limiting. Units which are started more than burst times within an interval time span are not permitted
	// to start any more. Use StartLimitIntervalSec= to configure the checking interval and StartLimitBurst= to configure
	// how many starts per interval are allowed.
	//
	// interval is a time span with the default unit of seconds, but other units may be specified, see systemd.time. The special
	// value "infinity" can be used to limit the total number of start attempts, even if they happen at large time intervals. Defaults
	// to DefaultStartLimitIntervalSec= in manager configuration file, and may be set to 0 to disable any kind of rate limiting.
	// burst is a number and defaults to DefaultStartLimitBurst= in manager configuration file.
	//
	// These configuration options are particularly useful in conjunction with the service setting Restart= (see systemd.service);
	...
	// however, they may still be restarted manually or from a timer or socket at a later point, after the interval has passed. From
	// that point on, the restart logic is activated again. systemctl reset-failed will cause the restart rate counter for a service
	// to be flushed, which is useful if the administrator wants to manually start a unit and the start limit interferes with that.
	// Rate-limiting is enforced after any unit condition checks are executed, and hence unit activations with failing conditions
	...
	//
	// This setting does not apply to slice, target, device, and scope units, since they are unit types whose activation may either
	// never fail, or may succeed only a single time.
	//
	...
	//
	// Use systemd-analyze for the complete list of known architectures.
	//
	...
	//
	// "smbios-field(field operator value)" matches systems with a SMBIOS field containing a certain value. field is the name
	// of the SMBIOS field exposed as "sysfs" attribute file below /sys/class/dmi/id/. operator is one of "<", "<=", ">=", ">",
	// "==", "<>" for version comparisons, "=" and "!=" for literal string comparisons, or "$=", "!$=" for shell-style glob comparisons.
	// value is the expected value of the SMBIOS field value (possibly containing shell style globs in case "$="/"!$=" is used).
	//
	// Added in version 249.
	ConditionFirmware systemdconf.Value
	...
	// test against a generic type of virtualization solution, or one of "qemu", "kvm", "amazon", "zvm", "vmware", "microsoft",
	// "oracle", "powervm", "xen", "bochs", "uml", "bhyve", "qnx", "apple", "sre", "openvz", "lxc", "lxc-libvirt", "systemd-nspawn",
	// "docker", "podman", "rkt", "wsl", "proot", "pouch", "acrn" to test against a specific implementation, or "private-users"
	// to check whether we are running in a user namespace. See systemd-detect-virt for a full list of known virtualization technologies
	// and their identifiers. If multiple virtualization technologies are nested, only the innermost is considered. The test
	...
	// or if prefixed with the exclamation mark, does not match. The argument must be a list of (potentially quoted) expressions.
	// Each expression starts with one of "=" or "!=" for string comparisons, "<", "<=", "==", "<>", ">=", ">" for version comparisons,
	// or "$=", "!$=" for a shell-style glob match. If no operator is specified, "$=" is implied.
	//
	...
	// ConditionCredential= may be used to check whether a credential by the specified name was passed into the service manager.
	// See System and Service Credentials for details about credentials. If used in services for the system service manager this
	// may be used to conditionalize services based on system credentials passed in. If used in services for the per-user service
	// manager this may be used to conditionalize services based on credentials passed into the unit@.service service instance
	// belonging to the user. The argument must be a valid credential name.
	//
	// Added in version 252.
	ConditionCredential systemdconf.Value
	...
	//	| measured-uki    | Unified Kernel Image with PCR 11 Measurements, as per systemd-stub. Added in     |
	//	|                 | version 255.                                                                     |
	//	+-----------------+----------------------------------------------------------------------------------+
	//
	...
	//
	// If the systemd.condition_needs_update= option is specified on the kernel command line (taking a boolean), it will override
	// the result of this condition check, taking precedence over any file modification time checks. If the kernel command line
	// option is used, systemd-update-done.service will not have immediate effect on any following ConditionNeedsUpdate=
	// checks, until the system is rebooted where the kernel command line option is not specified anymore.
	//
	// Note that to make this scheme effective, the timestamp of /usr/ should be explicitly updated after its contents are modified.
	// The kernel will automatically update modification timestamp on a directory only when immediate children of a directory
	// are modified; an modification of nested files will not automatically result in mtime of /usr/ being updated.
	//
	// Also note that if the update method includes a call to execute appropriate post-update steps itself, it should not touch
	// the timestamp of /usr/. In a typical distribution packaging scheme, packages will do any required update steps as part
	// of the installation or upgrade, to make package contents immediately usable. ConditionNeedsUpdate= should be used with
	// other update mechanisms where such an immediate update does not happen.
	//
	// Added in version 244.
	...
	// first time. This roughly means that /etc/ was unpopulated when the system started booting (for details, see "First Boot
	// Semantics" in machine-id). First Boot is considered finished (this condition will evaluate as false) after the manager
	// has finished the startup phase.
	//
	// This condition may be used to populate /etc/ on the first boot after factory reset, or when a new system instance boots up
	// for the first time.
	//
	// Note that the service manager itself will perform setup steps during First Boot: it will initialize machine-id and preset
	// all units, enabling or disabling them according to the systemd.preset settings. Additional setup may be performed via
	...
	//
	// If the systemd.condition_first_boot= option is specified on the kernel command line (taking a boolean), it will override
	...
	// Check whether given cgroup controllers (e.g. "cpu") are available for use on the system or whether the legacy v1 cgroup
	// or the modern v2 cgroup hierarchy is used.
	//
	// Multiple controllers may be passed with a space separating them; in this case, the condition will only pass if all listed
	// controllers are available for use. Controllers unknown to systemd are ignored. Valid controllers are "cpu", "io", "memory",
	// and "pids". Even if available in the kernel, a particular controller may not be available if it was disabled on the kernel
	// command line with cgroup_disable=controller.
	//
	// Alternatively, two special strings "v1" and "v2" may be specified (without any controller names). "v2" will pass if the
	// unified v2 cgroup hierarchy is used, and "v1" will pass if the legacy v1 hierarchy or the hybrid hierarchy are used. Note
	// that legacy or hybrid hierarchies have been deprecated. See systemd for more information.
	//
	// Added in version 244.
	...
	// optionally prefixed with a comparison operator "<", "<=", "=" (or "=="), "!=" (or "<>"), ">=", ">". On bare-metal systems
	// compares the amount of physical memory in the system with the specified size, adhering to the specified comparison operator.
	// In containers compares the amount of memory assigned to the container instead.
	//
	// Added in version 244.
	...
	// prefixed with a comparison operator "<", "<=", "=" (or "=="), "!=" (or "<>"), ">=", ">". Compares the number of CPUs in the
	// CPU affinity mask configured of the service manager itself with the specified number, adhering to the specified comparison
	// operator. On physical systems the number of CPUs in the affinity mask of the service manager usually matches the number
	// of physical CPUs, but in special and virtual environments might differ. In particular, in containers the affinity mask
	// usually matches the number of CPUs assigned to the container and not the physically available ones.
	//
	// Added in version 244.
	ConditionCPUs systemdconf.Value

	// Verify that a given CPU feature is available via the "CPUID" instruction. This condition only does something on i386 and
	// x86-64 processors. On other processors it is assumed that the CPU does not support the given feature. It checks the leaves
	// "1", "7", "0x80000001", and "0x80000007". Valid values are: "fpu", "vme", "de", "pse", "tsc", "msr", "pae", "mce", "cx8",
	// "apic", "sep", "mtrr", "pge", "mca", "cmov", "pat", "pse36", "clflush", "mmx", "fxsr", "sse", "sse2", "ht", "pni",
	// "pclmul", "monitor", "ssse3", "fma3", "cx16", "sse4_1", "sse4_2", "movbe", "popcnt", "aes", "xsave", "osxsave",
	// "avx", "f16c", "rdrand", "bmi1", "avx2", "bmi2", "rdseed", "adx", "sha_ni", "syscall", "rdtscp", "lm", "lahf_lm",
	// "abm", "constant_tsc".
	//
	// Added in version 248.
	ConditionCPUFeature systemdconf.Value

	// Verify that a specific "key=value" pair is set in the host's os-release.
	//
	// Other than exact string matching (with "=" and "!="), relative comparisons are supported for versioned parameters (e.g.
	// "VERSION_ID"; with "<", "<=", "==", "<>", ">=", ">"), and shell-style wildcard comparisons ("*", "?", "[]") are supported
	// with the "$=" (match) and "!$=" (non-match).
	//
	// If the given key is not found in the file, the match is done against an empty value.
	//
	// Added in version 249.
	ConditionOSRelease systemdconf.Value

	// Verify that the overall system (memory, CPU or IO) pressure is below or equal to a threshold. This setting takes a threshold
	// value as argument. It can be specified as a simple percentage value, suffixed with "%", in which case the pressure will be
	// measured as an average over the last five minutes before the attempt to start the unit is performed. Alternatively, the
	// average timespan can also be specified using "/" as a separator, for example: "10%/1min". The supported timespans match
	// what the kernel provides, and are limited to "10sec", "1min" and "5min". The "full" PSI will be checked first, and if not
	// found "some" will be checked. For more details, see the documentation on PSI (Pressure Stall Information) .
	//
	// Optionally, the threshold value can be prefixed with the slice unit under which the pressure will be checked, followed
	// by a ":". If the slice unit is not specified, the overall system pressure will be measured, instead of a particular cgroup's.
	//
	// Added in version 250.
	ConditionMemoryPressure systemdconf.Value
	...
	//
	// Added in version 250.
	ConditionCPUPressure systemdconf.Value
	...
	//
	// Added in version 250.
	ConditionIOPressure systemdconf.Value
	...
	//
	// Added in version 218.
	AssertCredential systemdconf.Value
	...
	//
	// Added in version 218.
	AssertCPUFeature systemdconf.Value
	...
	//
	// Added in version 218.
	AssertOSRelease systemdconf.Value
	...
	//
	// Added in version 218.
	AssertMemoryPressure systemdconf.Value
	...
	//
	// Added in version 218.
	AssertCPUPressure systemdconf.Value
	...
	//
	// Added in version 218.
	AssertIOPressure systemdconf.Value
}
...
type InstallSection struct {
	...
	// the .wants/, .requires/, or .upholds/ directory of each of the listed units when this unit is installed by systemctl enable.
	// This has the effect of a dependency of type Wants=, Requires=, or Upholds= being added from the listed unit to the current
	// unit. See the description of the mentioned dependency types in the [Unit] section for details.
	//
	// In case of template units listing non template units, the listing unit must have DefaultInstance= set, or systemctl enable
	// must be called with an instance name. The instance (default or specified) will be added to the .wants/, .requires/, or .upholds/
	// list of the listed unit. For example, WantedBy=getty.target in a service getty@.service will result in systemctl enable
	// getty@tty2.service creating a getty.target.wants/getty@tty2.service link to getty@.service. This also applies
	// to listing specific instances of templated units: this specific instance will gain the dependency. A template unit may
	// also list a template unit, in which case a generic dependency will be added where each instance of the listing unit will have
	// a dependency on an instance of the listed template with the same instance value. For example, WantedBy=container@.target
	// in a service monitor@.service will result in systemctl enable monitor@.service creating a container@.target.wants/monitor@.service
