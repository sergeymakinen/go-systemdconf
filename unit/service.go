...
type ServiceSection struct {
	...
	// Configures the mechanism via which the service notifies the manager that the service start-up has finished. One of simple,
	// exec, forking, oneshot, dbus, notify, notify-reload, or idle:
	//
	// If set to simple (the default if ExecStart= is specified but neither Type= nor BusName= are, and credentials are not used),
	// the service manager will consider the unit started immediately after the main service process has been forked off (i.e.
	// immediately after fork(), and before various process attributes have been configured and in particular before the new
	// process has called execve() to invoke the actual service binary). Typically, Type=exec is the better choice, see below.
	//
	// It is expected that the process configured with ExecStart= is the main process of the service. In this mode, if the process
	// offers functionality to other processes on the system, its communication channels should be installed before the service
	// is started up (e.g. sockets set up by systemd, via socket activation), as the service manager will immediately proceed
	// starting follow-up units, right after creating the main service process, and before executing the service's binary.
	// Note that this means systemctl start command lines for simple services will report success even if the service's binary
	// cannot be invoked successfully (for example because the selected User= does not exist, or the service binary is missing).
	//
	...
	// when the service's binary cannot be invoked successfully (for example because the selected User= does not exist, or the
	// service binary is missing). This type is implied if credentials are used (refer to LoadCredential= in systemd.exec for
	// details).
	//
	// If set to forking, the manager will consider the unit started immediately after the binary that forked off by the manager
	// exits. The use of this type is discouraged, use notify, notify-reload, or dbus instead.
	//
	// It is expected that the process configured with ExecStart= will call fork() as part of its start-up. The parent process
	// is expected to exit when start-up is complete and all communication channels are set up. The child continues to run as the
	// main service process, and the service manager will consider the unit started when the parent process exits. This is the
	// behavior of traditional UNIX services. If this setting is used, it is recommended to also use the PIDFile= option, so that
	// systemd can reliably identify the main process of the service. The manager will proceed with starting follow-up units
	...
	// the service will never enter "active" unit state, but will directly transition from "activating" to "deactivating" or
	// "dead", since no process is configured that shall run continuously. In particular this means that after a service of this
	// type ran (and which has RemainAfterExit= not set) it will not show up as started afterwards, but as dead.
	//
	// Behavior of dbus is similar to simple; however, units of this type must have the BusName= specified and the service manager
	// will consider the unit up when the specified bus name has been acquired. This type is the default if BusName= is specified.
	//
	// Service units with this option configured implicitly gain dependencies on the dbus.socket unit. A service unit of this
	// type is considered to be in the activating state until the specified bus name is acquired. It is considered activated while
	// the bus name is taken. Once the bus name is released the service is considered being no longer functional which has the effect
	// that the service manager attempts to terminate any remaining processes belonging to the service. Services that drop their
	// bus name as part of their shutdown logic thus should be prepared to receive a SIGTERM (or whichever signal is configured
	// in KillSignal=) as result.
	//
	// Behavior of notify is similar to exec; however, it is expected that the service sends a "READY=1" notification message
	// via sd_notify or an equivalent call when it has finished starting up. systemd will proceed with starting follow-up units
	// after this notification message has been sent. If this option is used, NotifyAccess= (see below) should be set to open access
	// to the notification socket provided by systemd. If NotifyAccess= is missing or set to none, it will be forcibly set to main.
	//
	// If the service supports reloading, and uses a signal to start the reload, using notify-reload instead is recommended.
	//
	// Behavior of notify-reload is similar to notify, with one difference: the SIGHUP UNIX process signal is sent to the service's
	// main process when the service is asked to reload and the manager will wait for a notification about the reload being finished.
	//
	// When initiating the reload process the service is expected to reply with a notification message via sd_notify that contains
	// the "RELOADING=1" field in combination with "MONOTONIC_USEC=" set to the current monotonic time (i.e. CLOCK_MONOTONIC
	// in clock_gettime) in μs, formatted as decimal string. Once reloading is complete another notification message must be
	// sent, containing "READY=1". Using this service type and implementing this reload protocol is an efficient alternative
	// to providing an ExecReload= command for reloading of the service's configuration.
	//
	...
	//
	// It is recommended to use Type=exec for long-running services, as it ensures that process setup errors (e.g. errors such
	// as a missing service executable, or missing user) are properly tracked. However, as this service type will not propagate
	// the failures in the service's own startup code (as opposed to failures in the preparatory steps the service manager executes
	// before execve()) and does not allow ordering of other units against completion of initialization of the service code itself
	// (which for example is useful if clients need to connect to the service through some form of IPC, and the IPC channel is only
	// established by the service itself — in contrast to doing this ahead of time through socket or bus activation or similar),
	// it might not be sufficient for many cases. If so, notify, notify-reload, or dbus (the latter only in case the service provides
	// a D-Bus interface) are the preferred options as they allow service program code to precisely schedule when to consider
	// the service started up successfully and when to proceed with follow-up units. The notify/notify-reload service types
	// require explicit support in the service codebase (as sd_notify() or an equivalent API needs to be invoked by the service
	// at the appropriate time) — if it is not supported, then forking is an alternative: it supports the traditional heavy-weight
	// UNIX service start-up protocol. Note that using any type other than simple possibly delays the boot process, as the service
	// manager needs to wait for at least some service initialization to complete. (Also note it is generally not recommended
	// to use idle or oneshot for long-running services.)
	//
	// Note that various service settings (e.g. User=, Group= through libc NSS) might result in "hidden" blocking IPC calls to
	// other services when used. Sometimes it might be advisable to use the simple service type to ensure that the service manager's
	// transaction logic is not affected by such potentially slow operations and hidden dependencies, as this is the only service
	// type where the service manager will not wait for such service execution setup operations to complete before proceeding.
	Type systemdconf.Value

	// Specifies when the manager should consider the service to be finished. One of main or cgroup:
	//
	// If set to main (the default), the service manager will consider the unit stopped when the main process, which is determined
	// according to the Type=, exits. Consequently, it cannot be used with Type=oneshot.
	//
	// If set to cgroup, the service will be considered running as long as at least one process in the cgroup has not exited.
	//
	// It is generally recommended to use ExitType=main when a service has a known forking model and a main process can reliably
	// be determined. ExitType= cgroup is meant for applications whose forking model is not known ahead of time and which might
	// not have a specific main process. It is well suited for transient or automatically generated services, such as graphical
	// applications inside of a desktop environment.
	//
	// Added in version 250.
	ExitType systemdconf.Value
	...
	//
	// Note that PID files should be avoided in modern projects. Use Type=notify, Type=notify-reload or Type=simple where possible,
	// which does not require use of PID files to determine the main process of a service and avoids needless forking.
	...
	//
	// Unless Type= is oneshot, exactly one command must be given. When Type=oneshot is used, this setting may be used multiple
	// times to define multiple commands to execute. If the empty string is assigned to this option, the list of commands to start
	// is reset, prior assignments of this option will have no effect. If no ExecStart= is specified, then the service must have
	// RemainAfterExit=yes and at least one ExecStop= line set. (Services lacking both ExecStart= and ExecStop= are not valid.)
	//
	// If more than one command is configured, the commands are invoked sequentially in the order they appear in the unit file.
	...
	// ExecStart=. Multiple command lines are allowed, regardless of the service type (i.e. Type=), and the commands are executed
	...
	// for Type=oneshot, the initial process exited successfully for Type=forking, "READY=1" is sent for Type=notify/Type=notify-reload,
	...
	// Optional commands that are executed before the commands in ExecStartPre=. Syntax is the same as for ExecStart=. Multiple
	// command lines are allowed, regardless of the service type (i.e. Type=), and the commands are executed one after the other,
	// serially.
	//
	...
	// commands will be skipped). Exit code of 0 or those matching SuccessExitStatus= will continue execution to the next commands.
	//
	...
	//
	// Note however that reloading a daemon by enqueuing a signal (as with the example line above) is usually not a good choice,
	// because this is an asynchronous operation and hence not suitable when ordering reloads of multiple services against each
	// other. It is thus strongly recommended to either use Type=notify-reload in place of ExecReload=, or to set ExecReload=
	// to a command that not only triggers a configuration reload of the daemon, but also synchronously waits for it to complete.
	// For example, dbus-broker uses the following:
	//
	...
	// specified in ExecStart=, ExecStartPre= or ExecStartPost= failed (and were not prefixed with "-", see above) or timed
	...
	// and left incompletely initialized data around. As the service's processes have likely exited already when the commands
	...
	// Configures the number of steps to take to increase the interval of auto-restarts from RestartSec= to RestartMaxDelaySec=.
	...
	RestartSteps systemdconf.Value

	// Configures the longest time to sleep before restarting a service as the interval goes up with RestartSteps=. Takes a value
	// in the same format as RestartSec=, or "infinity" to disable the setting. Defaults to "infinity".
	//
	// This setting is effective only if RestartSteps= is also set.
	//
	// Added in version 254.
	RestartMaxDelaySec systemdconf.Value
	...
	// logic. Defaults to DefaultTimeoutStartSec= set in the manager, except when Type=oneshot is used, in which case the timeout
	// is disabled by default (see systemd-system.conf).
	//
	// If a service of Type=notify/Type=notify-reload sends "EXTEND_TIMEOUT_USEC=…", this may cause the start time to be extended
	// beyond TimeoutStartSec=. The first receipt of this message must occur before TimeoutStartSec= is exceeded, and once
	// the start time has extended beyond TimeoutStartSec=, the service manager will allow the service to continue to start,
	// provided the service repeats "EXTEND_TIMEOUT_USEC=…" within the interval specified until the service startup status
	// is finished by "READY=1". (see sd_notify).
	//
	// Note that the start timeout is also applied to service reloads, regardless if implemented through ExecReload= or via the
	// reload logic enabled via Type=notify-reload. If the reload does not complete within the configured time, the reload will
	// be considered failed and the service will continue running with the old configuration. This will not affect the running
	// service, but will be logged and will cause e.g. systemctl reload to fail.
	//
	...
	// option. Second, it configures the time to wait for the service itself to stop. If it does not terminate in the specified time,
	...
	//
	// If a service of Type=notify/Type=notify-reload sends "EXTEND_TIMEOUT_USEC=…", this may cause the stop time to be extended
	// beyond TimeoutStopSec=. The first receipt of this message must occur before TimeoutStopSec= is exceeded, and once the
	// stop time has extended beyond TimeoutStopSec=, the service manager will allow the service to continue to stop, provided
	// the service repeats "EXTEND_TIMEOUT_USEC=…" within the interval specified, or terminates itself (see sd_notify).
	//
	// Added in version 188.
	...
	//
	// If a service of Type=notify/Type=notify-reload handles SIGABRT itself (instead of relying on the kernel to write a core
	// dump) it can send "EXTEND_TIMEOUT_USEC=…" to extended the abort time beyond TimeoutAbortSec=. The first receipt of this
	// message must occur before TimeoutAbortSec= is exceeded, and once the abort time has extended beyond TimeoutAbortSec=,
	...
	// as they terminate immediately after activation completed (use TimeoutStartSec= to limit their activation). Pass "infinity"
	// (the default) to configure no runtime limit.
	//
	// If a service of Type=notify/Type=notify-reload sends "EXTEND_TIMEOUT_USEC=…", this may cause the runtime to be extended
	// beyond RuntimeMaxSec=. The first receipt of this message must occur before RuntimeMaxSec= is exceeded, and once the runtime
	// has extended beyond RuntimeMaxSec=, the service manager will allow the service to continue to run, provided the service
	// repeats "EXTEND_TIMEOUT_USEC=…" within the interval specified until the service shutdown is achieved by "STOPPING=1"
	// (or termination). (see sd_notify).
	//
	...
	// this context, a clean exit means any of the following:
	//
	// exit code of 0;for types other than Type=oneshot, one of the signals SIGHUP, SIGINT, SIGTERM, or SIGPIPE; exit statuses
	// and signals specified in SuccessExitStatus=.
	//
	// If set to on-failure, the service will be restarted when the process exits with a non-zero exit code, is terminated by a signal
	// (including on core dump, but excluding the aforementioned four signals), when an operation (such as service reload) times
	// out, and when the configured watchdog timeout is triggered. If set to on-abnormal, the service will be restarted when the
	// process is terminated by a signal (including on core dump, excluding the aforementioned four signals), when an operation
	// times out, or when the watchdog timeout is triggered. If set to on-abort, the service will be restarted only if the service
	// process exits due to an uncaught signal not specified as a clean exit status. If set to on-watchdog, the service will be restarted
	// only if the watchdog timeout for the service expires. If set to always, the service will be restarted regardless of whether
	// it exited cleanly or not, got terminated abnormally by a signal, or hit a timeout. Note that Type=oneshot services will
	...
	// Added in version 254.
	//
	// If set to direct, the service transitions to the activating state directly during auto-restart, skipping failed/inactive
	// state. ExecStopPost= is still invoked. OnSuccess= and OnFailure= are skipped.
	//
	// This option is useful in cases where a dependency can fail temporarily but we do not want these temporary failures to make
	// the dependent units fail. Dependent units are not notified of these temporary failures.
	//
	// Added in version 254.
	//
	// If set to debug, the service manager will log messages that are related to this unit at debug level while automated restarts
	// are attempted, until either the service hits the rate limit or it succeeds, and the $DEBUG_INVOCATION=1 environment variable
	// will be set for the unit. This is useful to be able to get additional information when a service fails to start, without needing
	// to proactively or permanently enable debug level logging in systemd, which is very verbose. This is otherwise equivalent
	// to normal mode.
	//
	// Added in version 257.
	//
	// Added in version 254.
	RestartMode systemdconf.Value

	// Takes a list of exit status definitions that, when returned by the main service process, will be considered successful
	// termination, in addition to the normal successful exit status 0 and, except for Type=oneshot, the signals SIGHUP, SIGINT,
	// SIGTERM, and SIGPIPE. Exit status definitions can be numeric termination statuses, termination status names, or termination
	// signal names, separated by spaces. See the Process Exit Codes section in systemd.exec for a list of termination status
	// names (for this setting only the part without the "EXIT_" or "EX_" prefix should be used). See signal for a list of signal
	// names.
	//
	...
	// restarts, regardless of the restart setting configured with Restart=. Exit status definitions can be numeric termination
	// statuses, termination status names, or termination signal names, separated by spaces. Defaults to the empty list, so
	// that, by default, no exit status is excluded from the configured restart logic.
	//
	...
	//
	// This option may appear more than once, in which case the list of restart-preventing statuses is merged. If the empty string
	...
	//
	// Note that for Type=oneshot services, a success exit status will prevent them from auto-restarting, no matter whether
	// the corresponding exit statuses are listed in this option or not.
	//
	...
	//
	// Note that if the same socket unit is configured to be passed to multiple service units (via the Sockets= setting, see below),
	// and these services have different NonBlocking= configurations, the precise state of O_NONBLOCK depends on the order
	// in which these services are invoked, and will possibly change after service code already took possession of the socket
	// file descriptor, simply because the O_NONBLOCK state of a socket is shared by all file descriptors referencing it. Hence
	// it is essential that all services sharing the same socket use the same NonBlocking= configuration, and do not change the
	// flag in service code either.
	...
	// access to the notification socket when using Type=notify/Type=notify-reload or WatchdogSec= (see above). If those
	...
	// way. Application state can either be serialized to a file in RuntimeDirectory=, or stored in a memfd_create memory file
	// descriptor. Defaults to 0, i.e. no file descriptors may be stored in the service manager. All file descriptors passed to
	// the service manager from a specific service are passed back to the service's main process on the next service restart (see
	// sd_listen_fds for details about the precise protocol used and the order in which the file descriptors are passed). Any
	// file descriptors passed to the service manager are automatically closed when POLLHUP or POLLERR is seen on them, or when
	// the service is fully stopped and no job is queued or being executed for it (the latter can be tweaked with FileDescriptorStorePreserve=,
	// see below). If this option is used, NotifyAccess= (see above) should be set to open access to the notification socket provided
	// by systemd. If NotifyAccess= is not set, it will be implicitly set to main.
	//
	// The fdstore command of systemd-analyze may be used to list the current contents of a service's file descriptor store.
	//
	// Note that the service manager will only pass file descriptors contained in the file descriptor store to the service's own
	// processes, never to other clients via IPC or similar. However, it does allow unprivileged clients to query the list of currently
	// open file descriptors of a service. Sensitive data may hence be safely placed inside the referenced files, but should not
	// be attached to the metadata (e.g. included in filenames) of the stored file descriptors.
	//
	// If this option is set to a non-zero value the $FDSTORE environment variable will be set for processes invoked for this service.
	// See systemd.exec for details.
	//
	...
	// Takes one of no, yes, restart and controls when to release the service's file descriptor store (i.e. when to close the contained
	// file descriptors, if any). If set to no the file descriptor store is automatically released when the service is stopped;
	// if restart (the default) it is kept around as long as the unit is neither inactive nor failed, or a job is queued for the service,
	// or the service is expected to be restarted. If yes the file descriptor store is kept around until the unit is removed from
	// memory (i.e. is not referenced anymore and inactive). The latter is useful to keep entries in the file descriptor store
	// pinned until the service manager exits.
	//
	// Use systemctl clean --what=fdstore … to release the file descriptor store explicitly.
	//
	// Added in version 254.
	FileDescriptorStorePreserve systemdconf.Value
	...
	//
	// "path" is a path to a file or an AF_UNIX socket in the file system;"fd-name" is a name that will be associated with the file
	// descriptor; the name may contain any ASCII character, but must exclude control characters and ":", and must be at most 255
	// characters in length; it is optional and, if not provided, defaults to the file name;"options" is a comma-separated list
	// of access options; possible values are "read-only", "append", "truncate", "graceful"; if not specified, files will
	// be opened in rw mode; if "graceful" is specified, errors during file/socket opening are ignored. Specifying the same option
	// several times is treated as an error.
	//
	// The file or socket is opened by the service manager and the file descriptor is passed to the service. If the path is a socket,
	// we call connect() on it. See sd_listen_fds for more details on how to retrieve these file descriptors.
	//
	// This setting is useful to allow services to access files/sockets that they cannot access themselves (due to running in
	// a separate mount namespace, not having privileges, ...).
	//
	// This setting can be specified multiple times, in which case all the specified paths are opened and the file descriptors
	// passed to the service. If the empty string is assigned, the entire list of open files defined prior to this is reset.
	//
	// Added in version 253.
	OpenFile systemdconf.Value

	// Configures the UNIX process signal to send to the service's main process when asked to reload the service's configuration.
	// Defaults to SIGHUP. This option has no effect unless Type=notify-reload is used, see above.
	//
	// Added in version 253.
	ReloadSignal systemdconf.Value
}
