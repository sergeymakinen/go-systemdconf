...
type JournaldJournalSection struct {
	...
	// If the file system is nearly full and either SystemKeepFree= or RuntimeKeepFree= are violated when systemd-journald
	// is started, the limit will be raised to the percentage that is actually free. This means that if there was enough free space
	// before and journal files were created, and subsequently something else causes the file system to fill up, journald will
	// stop using more space, but it will not be removing existing files to reduce the footprint again, either. Also note that only
	// archived files are deleted to reduce the space occupied by journal files. This means that, in effect, there might still
	// be more space used than SystemMaxUse= or RuntimeMaxUse= limit after a vacuuming operation is complete.
	//
	...
	// eighth of the values configured with SystemMaxUse= and RuntimeMaxUse=, so that usually seven rotated journal files are
	// kept as history.
	//
	...
	// log buffer (kmsg), to the system console, or sent as wall messages to all logged-in users. These options take boolean arguments.
	...
	// (if that is enabled, see above). As argument, takes one of "emerg", "alert", "crit", "err", "warning", "notice", "info",
	// "debug", or integer values in the range of 0–7 (corresponding to the same levels). Messages equal or below the log level
	// specified are stored/forwarded, messages above are dropped. Defaults to "debug" for MaxLevelStore= and MaxLevelSyslog=,
	// to ensure that the all messages are stored in the journal and forwarded to syslog. Defaults to "notice" for MaxLevelKMsg=,
	// "info" for MaxLevelConsole=, and "emerg" for MaxLevelWall=. These settings may be overridden at boot time with the kernel
	// command line options "systemd.journald.max_level_store=", "systemd.journald.max_level_syslog=", "systemd.journald.max_level_kmsg=",
	...
	// Takes a boolean value. If enabled systemd-journal will turn on kernel auditing on start-up. If disabled it will turn it
	// off. If unset it will neither enable nor disable it, leaving the previous state unchanged. Note that this option does not
	// control whether systemd-journald collects generated audit records, it just controls whether it tells the kernel to generate
	// them. This means if another tool turns on auditing even if systemd-journald left it off, it will still collect the generated
